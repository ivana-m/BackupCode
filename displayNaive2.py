...
def draw(ymax, fileName, ax, plt, fig):
...
def main(directory):
    ...
    for (subdir, dirs, files) in os.walk(directory):
        for file in files:
            ...
            #offset = 1/numApps #this only works for naive
            ...
            for line in lines:
                ...
                plt.plot([0,1],[y,y], '0.75') #draw horizontal line to indicate power per task
                add_label(ax, text, x+w, y+h, font)
                x+=w
                y+=h
                #y+=offset #TODO y will be updated differently depending on algorithm 
            draw(powerCap, file, ax, plt, fig)        
            f.close()
    return 
